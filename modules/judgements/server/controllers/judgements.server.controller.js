...
exports.list = function(req, res) {
  Judgement.find().sort('-created').populate('user', 'displayName').exec(
    function(err, judgements) {
      if (err) {
        return res.status(400).send({
          message: errorHandler.getErrorMessage(err)
        });
...
exports.judgementsByProjectID = function(req, res, next, projectId) {
  Judgement.find()
    .where({
      project: projectId
    })
...
exports.judgementByID = function(req, res, next, id) {
  ...
  Judgement.findById(id).populate('user', 'displayName').exec(function(err,
    judgement) {
    if (err) {
      return next(err);
    } else if (!judgement) {
