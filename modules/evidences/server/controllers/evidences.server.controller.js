...
exports.list = function (req, res) {
  Evidence.find().sort('-created').populate('user', 'displayName').exec(function (err, evidences) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.evidenceByID = function (req, res, next, id) {
  ...
  Evidence.findById(id).populate('user', 'displayName').exec(function (err, evidence) {
    if (err) {
      return next(err);
    } else if (!evidence) {
